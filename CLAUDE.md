...
5. 次のコマンドを実行する:
   ...
   - `-VisibleCompileCheck`（2026-08-02追加）: 既定の`Z_CompileCheck`は
     ...
     **自動適用基準（2026-08-03追加、詳細はSKILL.md §5.6d）**: 下記いずれかに
     該当する場合、Claude Codeは`Update-AlarmMaster.ps1`実行前に本スイッチの
     使用を提案する（あくまで提案であり、画面表示を伴う実行そのものは
     引き続き必ずユーザーに一言断ってから行う。この基準はガードレールを
     緩めるものではない）：
       (a) `bas_staging\`への配置ファイルが2つ以上（複数モジュール同時反映）
       (b) 変更対象にSKILL.md §5.6d記載の「共通モジュール」を含む
       (c) `Check-VbaConventions.ps1`で新規のクロスモジュール参照が検出された
     いずれにも該当しない単純な単一モジュール変更では、既定どおり
     `Z_CompileCheck`のみで進めてよい。
